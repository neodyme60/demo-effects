...
void init()
{
  ...
  cam->setFrustumAndCamera(90.0, ((float)SCREEN_WIDTH) / ((float)SCREEN_HEIGHT), 0.1f, 50.0f, eye, look, up);
	
  cam->screen_width = SCREEN_WIDTH;
  cam->screen_height = SCREEN_HEIGHT;

  light = new WP_Light;
  light->setPosition(1.0, 1.0, 0.0, 0.0);
  light->diffuse_color = WP_Color((float)0.35,(float)0.75,(float)0.35);
  float ambient[] = {0.0, 0.0, 0.0, 1.0};
  glLightfv(GL_LIGHT0, GL_AMBIENT, ambient);  
  glLightfv(GL_LIGHT0, GL_DIFFUSE, light->diffuse_color.components);  
  glLightfv(GL_LIGHT0, GL_SPECULAR, light->diffuse_color.components);  
  state->enableLighting();
